...
function socketHandler(message, context) {
	let {ws, user, handshakeComplete} = context;
	let {username} = user;
		...
		if (message.op == 11 && message.type == 'IDENTIFY') {
			if (message.data.token) {
				...
				try {
					...
					const decodedToken = jwt.verify(message.data.token, process.env.JWT_SECRET) as Token;
			...
			const channelsO = Array.from(channels.values());
